...
func NewAddSignalImba(n *Nexus, exchange string, coin string, w_idx int) *AddSignalImba {
	s := AddSignalImba{}
	s.nexus = n
...
func (s *AddSignalImba) initParam(param map[string][]string) {
	s.num_signals += 1
	var val_f float64
	val, ok := param["c_lvl"]
	val_f, _ = strconv.ParseFloat(val[0], 64)
	if !ok {
		fmt.Println("no c_lvl for addsignalimba")
		os.Exit(1)
	}
	...
	val_f, _ = strconv.ParseFloat(val[0], 64)
	if !ok {
		fmt.Println("no c_size for addsignalimba")
		os.Exit(1)
	}
	...
	val_f, _ = strconv.ParseFloat(val[0], 64)
	if !ok {
		fmt.Println("no r_size for addsignalimba")
		os.Exit(1)
	}
	...
	val_f, _ = strconv.ParseFloat(val[0], 64)
	if !ok {
		fmt.Println("no levelcap for addsignalimba")
		os.Exit(1)
	}
	...
	val_f, _ = strconv.ParseFloat(val[0], 64)
	if !ok {
		fmt.Println("no horizon for addsignalimba")
		os.Exit(1)
	}
	...
	val_f, _ = strconv.ParseFloat(val[0], 64)
	if !ok {
		fmt.Println("no norm for addsignalimba")
		os.Exit(1)
	}
...
func (s *AddSignalImba) binary_search_upper(t time.Time) int {
	left := 0
	right := s.book.RecentAdds.Len() - 1
	res := right + 1
	for left <= right {
		mid := (left + right) / 2
		if t.Before(s.book.RecentAdds.At(mid).timestamp) {
			//if(s.book.RecentAdds.At(mid).timestamp > t){
			res = mid
			right = mid - 1
		} else {
			left = mid + 1
...
func (s *AddSignalImba) forward() {
	...
	for i := 0; i < len(s.norm); i++ {
		...
		askStrength := 0.0
		bidStrength := 0.0
		endidx := s.book.RecentAdds.Len() - 1
		startidx := s.horizon_idx[horizon]

		for k := startidx; k < endidx; k++ {
			side := s.book.RecentAdds.At(k).Side
			price := s.book.RecentAdds.At(k).Price
			//price_diff := math.Abs(curmid - price)
			price_diff := math.Abs(s.book.Mid() - price)
			lvl := s.book.RecentAdds.At(k).Level
			rsize := s.book.RecentAdds.At(k).OrigSize
			csize := s.book.RecentAdds.At(k).AddSize
